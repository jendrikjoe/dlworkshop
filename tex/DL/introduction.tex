...
\begin{itemize}
	\visible<2->{\item{It can save a lot of time of boooooring feature engineering and writing exceptions}}
...
\begin{minipage}{.4\textwidth}
	\begin{itemize}
		\visible<1->{\item The perceptron contains a weight($w$) for each input and a threshold($\Theta$)}
...
\begin{minipage}{.45\textwidth}
	...
	\end{axis}
	\end{tikzpicture}
	\only<3->{Data are linearly separable -> Can be divided by a plane}
...
\begin{minipage}{.45\textwidth}
	...
	\begin{axis}[xlabel={$x$}, ylabel={$y$},domain=0:1, width=\textwidth]
		...
		\only<5->{\addplot+[no marks, domain=0:.6, color=red] {-x+.6};}
		\only<6->{\addplot+[no marks, domain=0:.6, color=red] {-.8*x+.4};}
		\only<7->{\addplot+[no marks, domain=.64:.77, color=red] {-5*x+3.8};}
	\end{axis}
	\end{tikzpicture}
	\only<3->{Data is not linearly separable -> Can not be divided by a plane}
	}
...
\begin{itemize}
	...
	\visible<4->{\item The sigmoid allows us to convert the strict classification into a regression over the probability for each to belong to a certain class}
\end{itemize}
\end{minipage}
\hfill
\begin{minipage}{.45\textwidth}
...
\begin{itemize}
	...
	\visible<3->{\item Perceptron $\rightarrow$ can only split the data using a plan}
	...
	\visible<9->{\item What do we do if we want to store information about previous examples in the network}
\end{itemize}
